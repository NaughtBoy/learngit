package com.dartou.lib.network.updateApk;
...
public class UpdateService extends Service {
	...
	private int notificationId = 101862;
	...
	public int onStartCommand(Intent intent, int flags, int startId) {
		...
		notification.tickerText = appname + startDownload;
		...
		contentIntent = PendingIntent.getActivity(this, 0, new Intent(this,
				City.class), 0);
		notification.setLatestEventInfo(this, appname, "download 0%",
				contentIntent);
		...
		new Thread() {
			public void run() {
				try {
					...
					Message message = myHandler.obtainMessage(4, downloadFaild);
					myHandler.sendMessage(message);
				} catch (IOException e) {
					Message message = myHandler.obtainMessage(4, downloadFaild);
					myHandler.sendMessage(message);
			...
			if (msg != null) {
				...
				case 3:
					...
					notification.setLatestEventInfo(UpdateService.this,
							appname, downloaded + download_precent + "%",
							UpdateService.this.contentIntent);
					nm.notify(notificationId, notification);

					Log.v(notification + downloaded, download_precent + "%");
					break;
				case 4:
					nm.cancel(notificationId);
					break;
